...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($conta_id) || empty($valor)) {
        ...
        $mensagem_erro = "O valor do saque deve ser maior que zero.";
    } else {
        try {
            // Inicia a transação
            ...
            if ($valor > $saldo) {
                $mensagem_erro = "Saldo insuficiente para realizar o saque.";
            } else {
                ...
                $tipo_transacao_id = 2; // ID para saque
                ...
                echo "<div class='alert alert-success'>Saque realizado com sucesso!</div>";
            }
        } catch (PDOException $e) {
            // Rollback em caso de erro
            $pdo->rollBack();
            $mensagem_erro = "Erro ao realizar o saque: " . $e->getMessage();
        }
    }
}
